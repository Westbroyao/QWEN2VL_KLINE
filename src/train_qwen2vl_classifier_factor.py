...
def collect_eval_probs_tensor(
    ...
    with torch.inference_mode():
        for batch_idx, batch in enumerate(dataloader):
            ...
            probs = torch.softmax(logits.float(), dim=-1)        # [B, 3]
...
def rebuild_samples_from_image_dir(
    ...
    file_list = [f for f in os.listdir(image_dir) if f.endswith(suffix)]

    for fname in tqdm(file_list, desc=f"Rebuilding samples from {image_dir}"):
...
def run_model_on_samples(
    ...
    """
    输入 samples 列表，输出带概率的 DataFrame。
    """
    device = next(model.parameters()).device
    model.eval()
    ...
    with torch.inference_mode():
        for start in tqdm(
            ...
            total=total_batches,
            desc="Running classifier on samples",
        ):
...
def save_scores_dataframe(df_scores: pd.DataFrame, path: str):
    if path is None:
        return
    ...
    if path.endswith(".csv"):
        ...
        # 默认 parquet
        df_scores.to_parquet(path, index=False)
    print(f"[factor] raw scores saved to {path}, shape={df_scores.shape}")


def load_scores_dataframe(path: str) -> pd.DataFrame:
    if path.endswith(".csv"):
...
def build_factor_df_from_scores(
    ...
    df_scores["factor"] = (
        w_up * df_scores["p_up"]
        + w_down * df_scores["p_down"]
        + w_flat * df_scores["p_flat"]
    )
...
def save_factor_npz(factor_df: pd.DataFrame, path: str):
    ...
    dates = idx.to_numpy(dtype="datetime64[ns]")
...
def parse_args():
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["train", "factor_from_csv", "scores_to_npz"],
        default="train",
        help="train: 训练；factor_from_csv: 从CSV/图片生成因子；scores_to_npz: 直接从 raw scores 构建因子",
    )
    parser.add_argument(
        "--csv_glob",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="保存或加载模型概率输出的文件路径（parquet/csv）",
    )
    parser.add_argument(
        "--factor_w_up",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=-0.5,
        help="构建因子时 p_down 的权重",
    )
    parser.add_argument(
        "--factor_w_flat",
        type=float,
        default=0.0,
        help="构建因子时 p_flat 的权重",
    )
...
def main():
    ...
    if args.mode == "train":
        ...
        df_scores = None
        if args.raw_scores_path is not None and os.path.exists(args.raw_scores_path):
            print(f"[factor] found existing raw scores at {args.raw_scores_path}, skip inference.")
            df_scores = load_scores_dataframe(args.raw_scores_path)
        ...
        if df_scores is None:
            ...
            if args.raw_scores_path is not None:
                save_scores_dataframe(df_scores, args.raw_scores_path)

        factor_df = build_factor_df_from_scores(
            df_scores,
            w_up=args.factor_w_up,
            w_down=args.factor_w_down,
            w_flat=args.factor_w_flat,
        )
        save_factor_npz(factor_df, args.factor_npz_path)
        print("[factor] done.")
    elif args.mode == "scores_to_npz":
        if args.raw_scores_path is None or not os.path.exists(args.raw_scores_path):
            raise ValueError("scores_to_npz 模式需要有效的 --raw_scores_path")
        df_scores = load_scores_dataframe(args.raw_scores_path)
        factor_df = build_factor_df_from_scores(
            df_scores,
            w_up=args.factor_w_up,
